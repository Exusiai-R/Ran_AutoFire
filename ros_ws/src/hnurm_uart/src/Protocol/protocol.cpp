...
std::string Protocol::encode(const hnurm_interfaces::msg::VisionSendData& data)
{
  ...
  f_arr[2] = data.delta_pitch;
  f_arr[3] = data.delta_yaw;
  f_arr[4] = data.target_distance;
...
bool Protocol::decode(const std::string& s, hnurm_interfaces::msg::VisionRecvData& decoded_data)
{
  ...
  static uint16_t msg;
  static float f_arr[16];  // for further exploration, now receive 3 float data from STM32 (p,y,r)
  ...
  decoded_data.control_id        = f_arr[6];

  decoded_data.game_progress           = f_arr[7];
  decoded_data.stage_remain_time       = f_arr[8];
  decoded_data.current_hp              = f_arr[9];
  decoded_data.current_enemy_sentry_hp = f_arr[10];
  decoded_data.current_enemy_base_hp   = f_arr[11];
  decoded_data.current_virtual_shield  = f_arr[12];
  decoded_data.current_base_hp         = f_arr[13];
...
uint16_t Protocol::get_protocol_info_vision(
  uint8_t* rx_buf, uint16_t* flags_register, float* rx_data
)
{
  static protocol pro;
